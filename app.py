...
def get_s3_client():
    return boto3.client(
        ...
        region_name=AWS_REGION, # DOIT correspondre à la réalité du bucket
        config=Config(
            signature_version='s3v4',
            s3={'addressing_style': 'virtual'} # Force le style d'URL attendu par le navigateur
        )
    )
...
def download_file(key):
    try:
        ...
        # On extrait le nom du fichier (pour gérer les fichiers dans des sous-dossiers S3)
        ...
        url = s3.generate_presigned_url(
            ...
            Params={
                "Bucket": S3_BUCKET, 
                "Key": key,
                # C'est cette ligne magique qui force le navigateur à télécharger
